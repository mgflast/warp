...
- **`MTools reconstruct`**: Reconstruct an average from particle poses you already have, without refining anything. Takes a STAR file (RELION or M), a pixel size, a molecule diameter and one or more `.source` files, and produces half-maps plus a sharpened map using M's full reconstruction machinery — previously determined CTF, astigmatism, higher-order aberrations, image and volume deformation, dose weighting and magnification. Needs no reference and no pre-existing population; both are created fresh under `<root>/reconstructions/<name>/`, and nothing belonging to the input project is modified or versioned. Works for frame-series and tilt-series sources, which may be mixed in one run. Sharpening itself never needs a mask — pass `--mask` if you have one, otherwise a sphere of the molecule diameter is used for FSC estimation, which is unbiased but slightly pessimistic. Each run gets its own `<name>_<guid8>` directory, named the way species directories are, so repeated runs from the same particle file never collide. Half-set assignment honours `rlnRandomSubset`, falling back to `wrpRandomSubset` — which is what a WarpTools export from an M population carries — so the original split is preserved rather than randomly reassigned, which would break half-map independence and inflate the FSC.
- **`MTools postprocess`**: Re-estimate resolution, sharpen and locally filter an existing species or reconstruction, optionally with a different mask, working from the half-maps already on disk. The mask is used nowhere in extraction or back-projection — only in `CalculateResolutionAndFilter` — so trying a tighter mask takes seconds instead of a full re-reconstruction. `--sphere` reverts to the unbiased spherical mask; `--lowpass` filters to a fixed resolution instead of estimating one.
...
- **Frame-series reconstruction with zero refinement iterations produced a garbage map**: `Movie.PerformMultiParticleRefinement` initialised its per-particle magnification array to a zero matrix and only filled it inside the `NIterations > 0` block, while the back-projection at the end of the method reads it unconditionally. The back-projection kernel applies that matrix for 2-D input, so a zero matrix collapsed every Fourier sample onto DC. This affected `MCore --iter 0` on frame-series sources, silently and without any error. The array is now seeded with the item's actual magnification.
- **`SubsetMags` index mismatch in frame-series back-projection**: The per-particle magnification array was indexed positionally against the *filtered* list of in-frame particles while being built from the *unfiltered* one, throwing `IndexOutOfRangeException` as soon as any particle was dropped, and mispairing magnifications when more than one species was refined.
- **`VirtualConsole.ClearLastLine` threw when nothing had been logged yet**, which made the shared work-distribution helpers unusable from callers that never attach a console (tests, library consumers).
